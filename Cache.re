...
let to_bytes = str => {
  let buff = String.length(str) |> Buffer.create;
...
let write_record = (record, n, fd) => {
  seek_record(n, fd);
  ignore(Unix.write(fd, record.name |> to_bytes, 0, name_size));
  ignore(Unix.write(fd, record.address |> to_bytes, 0, addr_size));
};

let read_record = (record, n, fd) => {
  seek_record(n, fd);
  ignore(Unix.read(fd, record.name |> to_bytes, 0, name_size));
  ignore(Unix.read(fd, record.address |> to_bytes, 0, addr_size));
};
